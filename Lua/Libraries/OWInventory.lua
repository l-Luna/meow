...
return (function()
	...
	function self.CloseMenu()
		for i=0,2 do
			if self.itemMenuLabels[i] and self.itemMenuLabels[i].isactive then
				...
				self.itemMenuLabels[i] = {}
				self.itemMenuLabels[i].isactive = false
			end
		end
		for i=0,#self.itemLabels do
			if self.itemLabels[i] and self.itemLabels[i].isactive then
		...
		for i=0,#self.configOptionLabels do
			if self.configOptionLabels[i] and self.configOptionLabels[i].isactive then
				...
				self.configOptionLabels[i] = {}
				self.configOptionLabels[i].isactive = false
			end
		end
		for i=0,#self.configOptionValues do
			if self.configOptionValues[i] and self.configOptionValues[i].isactive then
				...
				self.configOptionValues[i] = {}
				self.configOptionValues[i].isactive = false
			end
		end
		if self.configLabel and self.configLabel.isactive then
			self.configLabel.Remove()
			self.configLabel = {}
			self.configLabel.isactive = false
		end
		self.uisoul.alpha = 0
	end
	...
	function self.UpdateMenu()
		...
		elseif self.selected == 3 then
			if Input.Up == 1 then
				self.configSelected = self.configSelected - 1
				self.configSelected = self.configSelected % #self.configs
			elseif Input.Down == 1 then
				self.configSelected = self.configSelected + 1
				self.configSelected = self.configSelected % #self.configs
			end
	...
	function self.CreateLabel(text)
		local label = CreateText("[instant][font:uidialog2]"..text, {0,0}, 999, "text", -1)
		label.HideBubble()
